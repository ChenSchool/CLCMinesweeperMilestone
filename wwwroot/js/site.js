...
$(document).ready(function () {
    ...
    // Add the right-click (contextmenu) event handler at the bottom
    $('button').on('contextmenu', function (e) {
        e.preventDefault(); // Prevent the default right-click menu

        // Get the ID of the button
        var buttonId = $(this).data('id'); // Ensure data-id is set in your HTML

        // Send AJAX request to toggle the flag
        $.ajax({
            type: 'POST',
            url: '/User/ToggleFlag', // The URL to hit in the controller
            data: { id: buttonId },  // Sending the button ID to the controller
            success: function (data) {
                // Update the board with the new flag state using the flag-specific partial view
                $('#gameBoard').html(data);  // This updates only the relevant part of the board
            }
        });
    });
});
